...
# Relative error
p1 <- ggplot(results1_df, aes(x = model, y = error, fill = model)) +
  geom_boxplot() +
...
p2 <- ggplot(results2_df, aes(x = model, y = error, fill = model)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 0.5) +
  geom_boxplot() +
  # facet_wrap(~setting, scales = "free") +
...
p3 <- ggplot(results3_df, aes(x = model, y = error, fill = model)) +
  geom_boxplot() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 0.5) +
  # facet_wrap(~setting, scales = "free") +
...
p4 <- ggplot(results4_df, aes(x = model, y = error, fill = model)) +
  geom_boxplot() +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 0.5) +
  # facet_wrap(~setting, scales = "free") +
  labs(title = "Spatiotemporal Effects and Time-Invariant Confounding", 
...
p7 <- ggplot(results_long,
  ...
  geom_boxplot(width = 0.7, outlier.size = 0.7, alpha = 0.8) +
  geom_hline(data = distinct(results_long, effect, true),
             aes(yintercept = true), linetype = "dashed",
             colour = "red", size = 0.4) +
  facet_wrap(~ effect, scales = "free_y") +
  labs(title = "Direct and Indirect Effect Estimates under Spatial Interference",
       x = "Estimator",
       y = "Estimated Effect",
       fill = "Estimator") +
  scale_fill_brewer(palette = "Set2") +
...
gen_error <- function(type, Tt, N, sigma_target,
  ...
  if (type == "gaussian") {
    ...
    # Use shape alpha; adjust scale so sd ≈ sigma_target
    alpha <- 5
...
# saveRDS(results_non_Gaussian_df, file = "sim_nonGaussian_estimate.rds")
```
...
# ggsave("sim_nonGaussian_estimate.pdf", p8, width = 6, height = 5, units = "in", dpi = 300)
```
