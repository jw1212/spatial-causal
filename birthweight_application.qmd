...
plot_effects_vi_gg <- function(sumres, Delta = 1.0,
  ...
  subtxt  <- paste(acd_txt, " | ", ate_txt)
  
  ## ── Palette & theme ------------------------------------------------------
  blue   <- "#003C7D"   # dark for medians
  fill95 <- alpha(blue, 0.15)

  theme_bw_mod <- theme_bw(base_size = 10, base_family = "serif") +
    theme(
      panel.grid.major  = element_line(size = .3, colour = "grey85"),
      panel.grid.minor  = element_blank(),
...
plots
# ggsave("bw_nonlinear_effects.pdf", plots, width = 6, height = 5, units = "in", dpi = 300)
