...
run_time_inter_analysis <- 
  ...
  reg <- lm(resY ~ resD + resD_lag, data = res_df)
  ...
  # IFE
  fe_fit <- interFE(resY ~ resD + resD_lag,
  ...
  ## Residuals to matrices -------------------------------------
  resD_mat <- matrix(dml_fit$resD, ncol = T_periods, byrow = TRUE)
  resY_mat <- matrix(dml_fit$resY, ncol = T_periods, byrow = TRUE)

  ## Stan (Factor model VB) ------------------------------------
  stan_data <- list(
    K = T_periods, Q = T_periods, N = N_units,
    M = M_factors, d = resD_mat,  y = resY_mat
  )
  sm_factor <- cmdstanr::cmdstan_model("stan/time_inter_factor.stan")
  vb_fit <- sm_factor$variational(data = stan_data, seed = seed, draws = vb_draws)
  vb_summary <- posterior::summarise_draws(
    vb_fit$draws(variables = c("beta0", "beta1", "beta2")),
    mean,
    ~quantile(.x, probs = c(0.025, 0.975))
    )
  ...
  ## 1.6  Return --------------------------------------------
  result_df <- 
    rbind(dml_summary[2:3,c(1:2,4:5)] %>% 
            mutate(model = "DML (NUC)", term = c("Direct", "Lagged")),
          vb_summary_df[2:3,] %>% 
            mutate(model = "FC", term = c("Direct", "Lagged")),
          fe_fit$est.table %>%
            as.data.frame() %>%
            tibble::rownames_to_column("term") %>%
            filter(term %in% c("resD", "resD_lag")) %>%
            mutate(
              term = c("Direct", "Lagged"),
              model = "IFE"
            ) %>%
...
```{r}
results_inter_all <- 
  run_time_inter_analysis(merged_bw_df_selected,
                          adj_cols= colnames(merged_bw_df_selected)[c(2,4:5,7,11:35)])
results_inter_all <- results_inter_all %>% mutate(setting = "All covariates")
```

```{r}
ggplot(results_inter, aes(x = model, y = estimate, color = term, shape = term)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper),
                width = 0.2,
                position = position_dodge(width = 0.5)) +
  facet_wrap(~ setting, nrow = 1) +
  labs(x = "Model", y = "Estimate", title = "Direct and Lagged Effects with 95% CI") +
  theme_minimal() +
