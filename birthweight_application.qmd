...
make_panel_arrays <- function(df, covar_cols = default_covars, years_allowed = 2018:2024,
  ...
  # keep covariates present in data
...
summarize_effect_curves <- function(result, Delta = 1.0,
  ...
  if (is.null(d_grid)) {
    d_all <- as.numeric(result$pan$d_mat)
    d_grid <- seq(quantile(d_all, 0.01), quantile(d_all, 0.99), length.out = 300)
  }
...
plot_effects_vi_gg <- function(sumres, Delta = 1.0,
  ...
  acd_txt <- sprintf("ACD = %.3f [%.3f, %.3f]",
                     sumres$acd[2], sumres$acd[1], sumres$acd[3])
  ate_txt <- sprintf("ATE(+%.1f) = %.3f [%.3f, %.3f]",
                     Delta, sumres$ate[2], sumres$ate[1], sumres$ate[3])

  p1 <- ggplot(sumres$dose_df, aes(d, med)) +
  ...
  p2 <- ggplot(sumres$te_df, aes(d, med)) +
    geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.15) +
    geom_line(size = 1) +
    geom_hline(yintercept = 0, linetype = 3) +
    labs(x = units_x, y = "Marginal effect  dY/dD",
         title = paste0(title_prefix, ": Marginal effect"),
         subtitle = paste(acd_txt, " · ", ate_txt)) +
    theme_minimal(base_size = 12)

  list(p_dose = p1, p_te = p2)
}
```
...
res_vi <- fit_zip_year_te(
  merged_bw_df_selected,
  covar_cols = colnames(merged_bw_df_selected)[c(2,4:5,7,11:35)], 
  M = 3L, L = 6L, degree = 3L
)
# saveRDS(res_vi, file = "bw_nonlinear_results.rds")

# Summarize on a +1 µg/m³ shift
sumres <- summarize_effect_curves(res_vi, Delta = 1.0, probs = c(0.025, 0.5, 0.975))
# saveRDS(sumres, file = "bw_nonlinear_estimated_effects.rds")
